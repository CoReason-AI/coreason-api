...
def test_simulate_agent_refined(mock_deps):
    ...
    response = client.post(
        "/v1/architect/simulate", json={"agent_code": {"code": "print('hi')"}, "input_data": {"x": 1}}
    )
...
def test_publish_agent_success(mock_deps):
    ...
    with (
        patch("coreason_api.routers.architect.ManifestValidator"),
        ...
        payload = {"slug": "my-agent", "name": "my-agent", "agent_manifest": {"version": "1.0"}}
...
def test_publish_agent_isomorphism_fail(mock_deps):
    ...
    payload = {"slug": "my-agent", "name": "Other Name", "agent_manifest": {}}
